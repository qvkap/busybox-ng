...
/*
 * Zstandard decompression applet for busybox
 *
 * Licensed under GPLv2 or later, see file LICENSE in this source tree.
 */
...
//config:	bool "unzstd (0.5 kb)"
//config:	default y
//config:	help
//config:	unzstd is used to decompress archives created by zstd.
//config:
//config:config ZSTDCAT
//config:	bool "zstdcat (0.5 kb)"
//config:	default y
//config:	help
...
#if ENABLE_UNZSTD || ENABLE_ZSTDCAT
...
int unzstd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	argv += optind;
	/* zstdcat? */
	if (ENABLE_ZSTDCAT && applet_name[0] == 'z')
		option_mask32 |= BBUNPK_OPT_STDOUT;

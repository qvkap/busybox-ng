...
#define unzstd_trivial_usage \
       "[-cfk] [FILE]..." \

#define unzstd_full_usage "\n\n" \
       "Decompress FILEs (or stdin)\n" \
     "\n	-c	Write to stdout" \
     "\n	-f	Force" \
     "\n	-k	Keep input files" \
...
#define zstdcat_trivial_usage \
       "[FILE]..." \

#define zstdcat_full_usage "\n\n" \
       "Decompress to stdout" \
...
#define cut_trivial_usage \
       "{-b|c LIST | -f"IF_FEATURE_CUT_REGEX("|F")" LIST [-d SEP] [-s]} [-D] [-O SEP] [FILE]..." \
...
#define sha384sum_trivial_usage \
	IF_FEATURE_MD5_SHA1_SUM_CHECK("[-c[sw]] ")"[FILE]..." \

#define sha384sum_full_usage "\n\n" \
       "Print" IF_FEATURE_MD5_SHA1_SUM_CHECK(" or check") " SHA384 checksums" \
	IF_FEATURE_MD5_SHA1_SUM_CHECK( "\n" \
     "\n	-c	Check sums against list in FILEs" \
     "\n	-s	Don't output anything, status code shows success" \
     "\n	-w	Warn about improperly formatted checksum lines" \
...
#define cmp_trivial_usage \
       "[-l|s] [-n NUM] FILE1 [FILE2" IF_DESKTOP(" [SKIP1 [SKIP2]]") "]" \
...
#define kmod_trivial_usage "[OPTIONS] [COMMAND]" \

#define kmod_full_usage "\n\nWrapper to execute external kmod tool" \

#define insmod_trivial_usage "FILE [ARGS]" \

#define insmod_full_usage "\n\nWrapper to external kmod" \

#define rmmod_trivial_usage "MODULE" \
...
#define modprobe_trivial_usage "MODULE" \

#define modprobe_full_usage "\n\nWrapper to external kmod" \

#define modinfo_trivial_usage "MODULE" \
...
#define curl_trivial_usage \
       "[OPTIONS] URL" \

#define curl_full_usage "\n\n" \
...
#define httpd_trivial_usage \
       ...
       " [-p [IP:]PORT]" \
       " [-M MAXCONN]" \
	IF_FEATURE_HTTPD_CGI(" [-K KILLSEC]") \
...
#define ssl_client_trivial_usage \
       "[-n SNI] { -s FD [-r FD] | HOST | -e PROG ARGS }" \

#define ssl_client_full_usage "" \

#define ssl_server_trivial_usage \
       "-f PRIVKEY_CERT.pem PROG ARGS" \

#define ssl_server_full_usage "" \
       "Inetd-style TLS server\n" \
...
#define telnet_trivial_usage \
	IF_FEATURE_TELNET_AUTOLOGIN("[-a] [-l USER] ")"HOST [PORT]" \

#define telnet_full_usage "\n\n" \
       "Connect to telnet server" \
	IF_FEATURE_TELNET_AUTOLOGIN("\n" \
...
#define wget_trivial_usage \
       "[-qc] [-O FILE] [-P DIR] [-j JOBS] [-U AGENT] [--no-check-certificate] URL..." \

#define wget_full_usage "\n\n" \
       "Minimal wget-compatible downloader\n" \
     "\n	-q		Quiet mode" \
...
#define vmstat_trivial_usage \
       "[-n] [INTERVAL [COUNT]]" \

#define vmstat_full_usage "\n\n" \
...
#define chrt_trivial_usage \
       "-m | [-rfobi] { -p [PRIO] PID | PRIO PROG ARGS }" \

#define chrt_full_usage "\n\n" \
       "Change scheduling priority and class (default RR) for a process\n" \
...
#define eject_trivial_usage \
       IF_FEATURE_EJECT_SCSI("[-s] ") "[-t] [-T] [DEVICE]" \
...
#define fdisk_trivial_usage \
       ...
       "[-C CYLINDERS] [-H HEADS] [-S SECTORS] [-b SSZ] [-t PARTTYPE] DISK" \
...
#define findfs_full_usage "\n\n" \
       "Find filesystem device based on label or UUID" \
...
#define xxd_trivial_usage \
       "[-ri] [-ps] [-g N] [-c N] [-l LEN] [-s OFS] [-o OFS] [FILE]" \
...
#define lsblk_trivial_usage \
       "[BLOCKDEV]..." \

#define lsblk_full_usage "\n\n" \
       "List block devices" \
...
#define uuidgen_trivial_usage \
       "" \

#define uuidgen_full_usage "\n\n" \
...
#define udhcpc6_trivial_usage \
       ...
       "	[-p PIDFILE]"IF_FEATURE_UDHCP_PORT(" [-P PORT]")" [-mldo] [-r IPv6] [-x OPT:VAL]... [-O OPT]..." \
